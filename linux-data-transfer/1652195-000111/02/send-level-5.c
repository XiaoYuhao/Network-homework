...
int main(){
	...
	int tcp_offset=atoi(m_mapConfigInfo["tcp_offset"].c_str());
	int extern_len=(tcp_offset-5)*4;
	...
	struct shared_memory* shared=create_share_memory(1);//在应用层和网络层之间创建共享内存
	shared->ready=0;
	memcpy(&shared->data[14+20+20+extern_len],data,num);//裸数据
